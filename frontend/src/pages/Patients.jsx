        ...
        <div className="space-y-6">
            ...
            {showForm && (
                <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200">
                    <h2 className="text-lg font-semibold mb-4 text-slate-800">New Patient Registration</h2>
                    <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <input
                            type="text" placeholder="Full Name" required
                            className="p-2 border border-slate-200 rounded-lg focus:ring-2 focus:ring-primary-400 focus:outline-none"
                        ...
                        <input
                            ...
                            className="p-2 border border-slate-200 rounded-lg md:col-span-2 focus:ring-2 focus:ring-primary-400 focus:outline-none"
                            value={formData.address} onChange={e => setFormData({ ...formData, address: e.target.value })}
                        />
                        <textarea
                            placeholder="Medical History (Optional)"
                            className="p-2 border border-slate-200 rounded-lg md:col-span-2 h-24 focus:ring-2 focus:ring-primary-400 focus:outline-none"
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-left">
                        ...
                        <tbody className="divide-y divide-slate-100">
                            {loading ? (
                                <tr><td colSpan="5" className="px-6 py-8 text-center text-slate-500">Loading patients...</td></tr>
                            ) : filteredPatients.length === 0 ? (
                                <tr><td colSpan="5" className="px-6 py-8 text-center text-slate-500">No patients found.</td></tr>
                            ) : (
                                filteredPatients.map((patient) => (
                                    <tr key={patient.id} className="hover:bg-slate-50/50 transition-colors">
                                        <td className="px-6 py-4 font-medium text-slate-900">{patient.name}</td>
                                        <td className="px-6 py-4 text-slate-600">{patient.contactNo}<br /><span className="text-xs text-slate-400">{patient.email}</span></td>
